...
const { search } = require("./utils/function");
...
const master = backup.createNewTransaction(
    1000000,
    "system-reward",
    public_key
);
...
app.use("/styles", express.static(__dirname + "/views/assets")); //allow css in invitation page (public)
...
io.on("connection", (socket) => {
    ...
    /*
     * Title: Broadcast Transaction section
    ...
    app.post("/transaction/broadcast", (req, res) => {
        ...
        if (
            ...
            const privateKey_Is_Valid =
                sha256(req.body.privKey) === req.body.sender;
        ...
        if (amount > 0 && flag === true) {
            ...
            nodes.forEach((socketNode) => {
                ...
                // io.sockets.connected[(socketNode.socketId).toString()].pendingTransactions = socketNode.pendingTransactions;//add property to socket
                // pt = socketNode.pendingTransactions;
            });
            pt = backup.getPendingTransactions();
    ...
    app.post("/mine", (req, res) => {
        ...
        const newBlock = backup.createNewBlock(
            nonce,
            previousBlockHash,
            blockHash
        ...
        rp(requestOptions)
            .then((data) => {
                //reward the miner after mining succeed and new block already created
                const requestOptions = {
     ...
     * Description: emit all sockets - a message to all sockets for mining operation succeed
     */
                ...
                let promise = new Promise(function (resolve, reject) {
                    dbo.collection("users")
                        ...
                        .toArray(function (err, result) {
                            ...
                            if (err) throw err;
                            // console.log(result);
                            toChange = result[0].inv; //set the variable to the num of available invitations
                ...
                promise.then(
                    function (result) {
                        if (toChange === 0) {
                            ...
                            let newInvite = {
                                $push: { availableInvitations: invitationID },
                            };
                                    ...
                                    dbo.collection("users").updateOne(
                                        ...
                                        function (err, res2) {
                                            ...
                                            var mailOptions = {
                                                ...
                                                to: email,
                                                subject:
                                                    "You have been invited to HCMUS-COIN",
                                                text:
                                                    "You have received an invitation to join HCMUS-COIN \n To activate the account, click the attached link:\n\n" +
                                                    uri,
                                            };
    ...
    MongoClient.connect(url, function (err, db) {
        ...
        dbo.collection("users")
            ...
            .toArray(function (err, result) {
                ...
                console.log(result);
                if (result.length === 0) {
                    res.render(__dirname + "/views/error.ejs"); //respond error page
                } else {
                    const priK = uuid().split("-").join(""); //privateKey
                    const pubK = sha256(priK); //publicKey
                    res.render(__dirname + "/views/invitation.ejs", {
...
/*  -when user press confirm register button (del invitation, make transactions to sender and recipient-  */
///////////////////////////////////////////////////////////////////////////////////////////////
app.post("/confirmRegister", (req, res) => {
    ...
    MongoClient.connect(url, function (err, db) {
        ...
        //find sender of invitation and make his invitation to unavailable (remove from db)
        dbo.collection("users")
            .find(query)
            .toArray(function (err, result) {
                //find the sender in db
                ...
                let pullInvite = {
                    $pull: { availableInvitations: invitationID },
                };
                dbo.collection("users").updateOne(
                    query,
                    pullInvite,
                    function (err, res) {
                        ...
                        if (err) throw err;
                        console.log(
                            res.result.nModified + " document(s) updated"
                        );
                        db.close();
                    }
                );
